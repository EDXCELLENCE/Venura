...
export default function ProgramDetailTemplate({ programData }) {
  ...
  const handleDownloadSyllabus = () => {
    ...
    const lines = [
      ...
      "",
      "Topics:",
      ...(programData.topics || []).map(
        (topic, idx) => `${idx + 1}. ${topic.title} - ${topic.description}`,
      ),
      "",
      "Curriculum:",
  ...
  const availableSections = [
    {
      ...
      icon: "🧠",
      enabled: !!programData?.topics,
    },
    {
      key: "prerequisites",
      label: "Prerequisites",
      icon: "📌",
      enabled: !!programData?.prerequisites,
    },
    {
      key: "curriculum",
      label: "Curriculum",
      icon: "🗺️",
      enabled: !!programData?.curriculum,
    },
    {
      key: "projects",
      label: "Projects",
      icon: "🛠️",
      enabled: !!programData?.projects,
    },
    { key: "pricing", label: "Pricing", icon: "💳", enabled: true },
    {
      ...
      icon: "🎓",
      enabled: !!programData?.certifications,
    },
    { key: "outcomes", label: "Career Outcomes", icon: "📈", enabled: true },
    {
      ...
      label: "Instructors",
      icon: "👨‍🏫",
      enabled: !!programData?.instructors,
    },
    {
      key: "reviews",
      label: "Reviews",
      icon: "⭐",
      enabled: !!programData?.reviews,
    },
    {
      key: "comparison",
      label: "Why Choose Venura?",
      icon: "⚖️",
      enabled: !!programData?.competitorComparison,
    },
    {
      key: "certificate",
      label: "Certificate",
      icon: "📜",
      enabled: !!programData?.certificatePreview,
    },
          ...
          {source.map((item, idx) => (
            <Fragment key={`${item.label}-${idx}`}>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="underline hover:text-[#0A2342]">
                {item.label}
              </a>
              {idx < source.length - 1 ? ", " : ""}
            </Fragment>
  ...
  useEffect(() => {
    if (!activeFeatureSection && defaultSectionKey) {
      ...
      const isValidSaved =
        savedSection &&
        availableSections.some((section) => section.key === savedSection);
  ...
  const foundationModules = hasPhaseData
    ? curriculum.filter((module) => module.phase === "foundation")
    : curriculum.slice(0, oneThird);
  const intermediateModules = hasPhaseData
    ? curriculum.filter((module) => module.phase === "intermediate")
    : curriculum.slice(oneThird, oneThird * 2);
  const advancedModules = hasPhaseData
    ? curriculum.filter((module) => module.phase === "advanced")
    : curriculum.slice(oneThird * 2);
  const seatsAvailable = programData.batchInfo?.seatsAvailable;
  const totalSeats = programData.batchInfo?.totalSeats;
  const enrolledCount =
    typeof seatsAvailable === "number" && typeof totalSeats === "number"
      ? Math.max(totalSeats - seatsAvailable, 0)
      : null;

  const contentCardClass =
    "motion-card bg-gradient-to-br from-white to-slate-50 rounded-2xl p-4 border border-slate-200 shadow-sm hover:shadow-md transition-all";
  const sectionTitleClass =
    "text-2xl font-extrabold text-[#0A2342] mb-5 tracking-tight";
  ...
  const renderSidebarContent = () => {
      ...
      return (
        <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center">
          ...
          <p className="text-xs text-slate-500 mt-1">
            Use the sidebar options to explore the program details.
          </p>
        </div>
      );
    }
          ...
          <div className="grid md:grid-cols-3 gap-4 motion-grid">
            <div className={`${contentCardClass} border-t-4 border-green-500`}>
              ...
              <div className="space-y-2">
                {(programData.prerequisites?.required || []).map((i, idx) => (
            ...
            <div className={`${contentCardClass} border-t-4 border-[#FF7A00]`}>
              ...
              <div className="space-y-2">
                {(programData.prerequisites?.recommended || []).map(
                  (i, idx) => (
                    <p key={idx} className="text-sm text-slate-600">
                      • {i}
                    </p>
            ...
            <div className={`${contentCardClass} border-t-4 border-[#0A2342]`}>
              ...
              <div className="space-y-2">
                {(programData.prerequisites?.idealFor || []).map((i, idx) => (
                  <p key={idx} className="text-sm text-slate-600">
                    • {i}
                  </p>
                ))}
    ...
    if (activeFeatureSection === "curriculum") {
      const modules =
        activeCurriculumTab === "foundation"
          ? foundationModules
          : activeCurriculumTab === "intermediate"
            ? intermediateModules
            : advancedModules;
      return (
        <div>
          ...
          <div className="flex gap-2 mb-4 flex-wrap">
            <button
              onClick={() => setActiveCurriculumTab("foundation")}
              className={`px-4 py-2 rounded-lg text-sm font-semibold ${activeCurriculumTab === "foundation" ? "bg-[#FF7A00] text-white" : "bg-white border border-[#FF7A00]/40 text-[#FF7A00]"}`}>
              Foundation
            </button>
            <button
              onClick={() => setActiveCurriculumTab("intermediate")}
              className={`px-4 py-2 rounded-lg text-sm font-semibold ${activeCurriculumTab === "intermediate" ? "bg-[#14345d] text-white" : "bg-white border border-[#14345d]/30 text-[#14345d]"}`}>
              Intermediate
            </button>
            <button
              onClick={() => setActiveCurriculumTab("advanced")}
              className={`px-4 py-2 rounded-lg text-sm font-semibold ${activeCurriculumTab === "advanced" ? "bg-[#0A2342] text-white" : "bg-white border border-[#0A2342]/30 text-[#0A2342]"}`}>
              Advanced
            </button>
          </div>
          <div className="grid md:grid-cols-2 gap-4 motion-grid">
              ...
              <div key={idx} className={contentCardClass}>
                ...
                <div className="space-y-1 mt-3">
                  {(module.topics || []).map((topic, topicIdx) => (
                    <p key={topicIdx} className="text-xs text-slate-600">
                      • {topic}
                    </p>
                  ))}
                </div>
              ...
              <div key={plan.name} className={contentCardClass}>
                ...
                <div className="space-y-1 mt-3">
                  {(plan.features || []).map((feature, featureIdx) => (
                    <p key={featureIdx} className="text-xs text-slate-600">
                      • {feature}
                    </p>
                  ))}
                </div>
      ...
      return (
        <div>
          ...
          <div className="grid md:grid-cols-2 gap-4 motion-grid">
            ...
            <div className={contentCardClass}>
              ...
              <div className="flex flex-wrap gap-2">
                {(programData.hiringCompanies || []).map((company, idx) => (
                  <span
                    key={idx}
                    className="text-xs bg-slate-100 text-slate-700 px-2 py-1 rounded-full">
                    {company}
          ...
          <div className={`${contentCardClass} mt-4`}>
            ...
            <div className="flex flex-wrap gap-2">
              {(programData.skills || []).map((skill, idx) => (
                <span
                  key={idx}
                  className="text-xs bg-[#FF7A00] text-white px-2 py-1 rounded-full">
                  {skill}
                </span>
              ))}
            </div>
          ...
          {programData.growthStats && (
            <div className="grid md:grid-cols-3 gap-3 mt-4 motion-grid">
              {programData.growthStats.map((stat, idx) => (
                <div key={idx} className={contentCardClass}>
                  <p className="text-lg font-extrabold text-[#FF7A00]">
                    {stat.value}
                  </p>
              ...
              <tbody>
                {(programData.competitorComparison || []).map((row, idx) => (
                  <tr key={idx} className="border-t border-slate-200">
                    ...
                    <td className="px-4 py-2 text-sm text-[#FF7A00] font-bold">
                      {row.venura}
                    </td>
                    <td className="px-4 py-2 text-sm text-slate-600">
                      {row.coursera}
      ...
      return (
        <div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-3 shadow-sm">
            <img
              src={programData.certificatePreview}
        ...
        <div>
          ...
          <div className="space-y-3 motion-grid">
            {(programData.faqs || []).map((faq, idx) => (
              <details
                key={idx}
                className="bg-gradient-to-br from-white to-slate-50 rounded-xl p-4 border border-slate-200 shadow-sm hover:shadow-md transition-all">
          ...
          <div className="mt-4">
            <Link
              to="/contact"
              className="inline-flex items-center gap-2 bg-[#0A2342] hover:bg-[#14345d] text-white font-bold py-2.5 px-5 rounded-lg transition-all text-sm">
    ...
    return (
      <p className="text-sm text-slate-700">Section content unavailable.</p>
    );
  };
  ...
  return (
    <div className="w-full min-h-screen bg-white">
      ...
      <main>
        <section
          className="relative overflow-hidden bg-[#0A2342] text-white"
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-6 py-20 lg:py-28">
            <div className="max-w-3xl">
              <Link
                to="/programs"
                className="inline-flex items-center text-slate-300 hover:text-white text-sm font-semibold mb-6 transition-colors">
                ← Back to Programs
              </Link>
              <p className="text-[#FFB273] text-sm font-bold uppercase tracking-widest mb-3">
                {programData.category}
              </p>
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white mb-3 leading-tight">
                {programData.title}
              </h1>
              <p className="text-[#FF7A00] text-lg md:text-xl font-semibold mb-5">
                {programData.subtitle}
              </p>
              <p className="max-w-2xl text-slate-200 leading-relaxed mb-6">
                {programData.description}
              </p>

              <div className="flex flex-wrap gap-2 mb-5">
                {programData.duration && (
                  <span className="bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm font-semibold backdrop-blur-sm">
                    {programData.duration}
                  </span>
                )}
                {programData.level && (
                ...
                {programData.batchInfo?.nextBatchDate && (
                  <span className="bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm font-semibold backdrop-blur-sm">
                    Next Batch: {programData.batchInfo.nextBatchDate}
                  </span>
                )}
              </div>
              ...
              {typeof seatsAvailable === "number" && (
                <div className="mb-6 max-w-xl rounded-xl border border-[#FF7A00]/40 bg-[#FF7A00]/15 p-4 backdrop-blur-sm">
                  <p className="text-[#ffd0aa] font-bold text-sm">
                    🔥 Only {seatsAvailable} seats left!
                  </p>
                  {typeof totalSeats === "number" && (
                    <p className="text-slate-200 text-sm mt-1">
                      Seats left: {seatsAvailable} of {totalSeats}
                      {enrolledCount !== null
            ...
            <div className="max-w-7xl mx-auto">
              <div className="text-center mb-8">
                ...
                <p className="text-slate-600 text-lg">
                  Structured live learning with mentor support
                </p>
              </div>
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-gradient-to-br from-slate-50 to-white rounded-2xl p-6 shadow-lg border-l-4 border-[#d46300]">
                  ...
                  <p className="text-slate-700 font-semibold mb-2">
                    {programData.learningFormat.liveSessionDuration}
                  </p>
                  <p className="text-slate-600 text-sm">
                    {programData.learningFormat.additionalSupport}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto grid lg:grid-cols-4 gap-4">
              <aside className="hidden lg:block lg:col-span-1 bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
                ...
                <p className="text-xs text-slate-500 mb-3">
                  Choose one section to focus on
                </p>
                <div className="space-y-2">
                  {availableSections.map((section) => (
                    <button
              ...
              <div className="lg:col-span-3 col-span-1 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                    ...
                    <span>
                      {
                        availableSections.find(
                          (section) => section.key === activeFeatureSection,
                        )?.icon
                      }
                    </span>
                    <span>
                ...
                <div
                  key={activeFeatureSection || "empty"}
                  className="sidebar-content-panel section-switch-anim">
