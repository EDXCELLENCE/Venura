...
export default function LearningPathsSection() {
  ...
  return (
    <section
      id="learning-paths"
      className="relative px-6 py-14 md:py-16 bg-[linear-gradient(180deg,#f8fbff_0%,#f4f8fd_45%,#f6f8fb_100%)] border-t-2 border-[#0A2342] overflow-hidden">
      <div
      ...
      <div className="mx-auto max-w-7xl">
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          ...
          <p className="mt-4 text-base text-slate-600 md:text-lg">
            Swipe through focused tracks and choose the one that matches your
            next career move.
          </p>
          <div className="mt-4 flex flex-wrap items-center justify-center gap-2.5">
            <span className="rounded-full border border-[#0A2342]/10 bg-white px-3 py-1 text-xs font-semibold text-slate-700">
              6 Career Tracks
            </span>
            <span className="rounded-full border border-[#0A2342]/10 bg-white px-3 py-1 text-xs font-semibold text-slate-700">
              Mentor Guided
          ...
          <p className="mt-3 text-xs font-medium uppercase tracking-[0.16em] text-slate-400">
            Swipe or scroll to explore tracks
          </p>
        </div>

        <div
          ref={scrollerRef}
          className="relative z-10 mt-10 overflow-x-auto pb-4 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          ...
          <div className="flex gap-5 pr-4 min-w-max snap-x snap-mandatory">
            {paths.map((path, index) => (
              <article
                ...
                <div
                  className={`relative h-36 bg-gradient-to-br ${path.gradient}`}>
                  {path.image ? (
                    <img
                      src={path.image}
                      alt={path.title}
